...
// Argument Parser
const cli = cac("mkatlas").version("1.0.0")
...
cli.command("[...files]", "Generate a Texture Atlas from a list of files. Glob notation accepted.")
    ...
    .option("-e, --extentions [out]", "Choose the image types accepted (bmp,png,jpg) as a list. (E.g: [bmp,png])", { default: "[bmp]" })
...
const imageList = await fg(imagePaths, { absolute: true, markDirectories: true });
...
for (let filePath of imageList) {
    ...
    //let imageData, imageMetadata, rawData;
    // if (path.extname(filePath).replace(".","") === "bmp") {
    //     const { image: _imageData, data: _rawData } = await transform(bmpFile, "#fc0fc0", true)

    //     imageData = _imageData
    //     rawData = _rawData
    //     imageMetadata = await imageData.metadata()
    // } else {
    //     imageData = sharp(filePath)
    //     imageMetadata = await imageData.metadata()
    // }
    const { image: _imageData, data: rawData } = await transform(bmpFile, "#fc0fc0", true)
    const imageData = _imageData.trim()
...
GrowingPacker.sorters.now(sorter,imageTable)
packer.fit(imageTable)
let { w,h } = packer.root
...
for (const i of imageTable) {
    const image = i.image//.trim()
...
// console.log(compositeImageTable)
...
const offsets = {
    atlas: imageTable.reduce((acc, cur) => cur.fit ? {...acc, [cur.id]: { x: cur.fit.x, y: cur.fit.y }} : acc, {})
}
//console.log("OFFSETS", offsets)

// Save atlas to temporary PNG
const atlasPNGPath = path.join(tempPngPath, `atlas.png`)
...
await fsp.copyFile(path.join(tempBmpPath, `atlas.bmp`), path.join(outputPath, "atlas.bmp")) // path.join(__dirname, "../../", "atlas.bmp")

if (args.options.offsetFormat === "json") {
    await fsp.writeFile(path.join(outputPath, "offsets.json"), JSON.stringify(offsets, null, 4)) // path.join(__dirname, "../../", "offsets.json")
        ...
        offsets = Offsets {
            ${
                Object.entries(offsets).map(([key,value]) => 
                    `${key} = Map.fromList [${
                        Object.entries(value).map(([k2,v2]) => `("${k2}", OffsetEntry ${v2.x} ${v2.y})`).join(",")
                    }]`
                ).join(",\n            ")
            }
        }
